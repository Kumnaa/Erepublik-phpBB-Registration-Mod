...
* @package ucp
*/
class cccp_register_erepublik_admin
{
	var $u_action;
	...
	function main($id, $mode)
	{
		...
		if ($auth->acl_get('a_'))
		{
			...
			$cccp_email = request_var('email','');
			$cccp_id = request_var('id','');
			$cccp_username = request_var('username','');
			
			if (strlen($cccp_email) > 0 && strlen($cccp_id) > 0 && strlen($cccp_username) > 0)
			{
				$sql = 'SELECT user_password, user_email, group_id, user_timezone,
						...
						WHERE user_email = '". $db->sql_escape($cccp_email) ."'";
				$result = $db->sql_query($sql);
				$new_user_data = $db->sql_fetchrow($result);
				if (count($new_user_data))
				{
					$new_password = $new_user_data['new_password'];
					unset($new_user_data['new_password']);
					$new_user_data["username"] = $cccp_username;
					$new_user_data["erepublik_id"] = $cccp_id;
					...
					if (count($user_name_validator) > 0)
					{
